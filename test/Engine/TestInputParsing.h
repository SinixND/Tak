...
void testParseInputDirection( void )
{
    ...
    UIData uiData = newUIData();
    int const boardSize = 8;

    TEST_ASSERT_EQUAL_INT( false, parseDirectionInputToCommand( &command, &inputBuffer, boardSize, &uiData ) );
    ...
    TEST_ASSERT_EQUAL_INT( true, parseDirectionInputToCommand( &command, &inputBuffer, boardSize, &uiData ) );
...
void testParseInputFirstDropAmount( void )
{
    ...
    TEST_ASSERT_EQUAL_INT( false, parseFirstDropAmountInputToCommand( &command, &inputBuffer, 8, &uiData ) );
    ...
    TEST_ASSERT_EQUAL_INT( true, parseFirstDropAmountInputToCommand( &command, &inputBuffer, 8, &uiData ) );
...
void testParseInputDropAmount( void )
{
    ...
    TEST_ASSERT_EQUAL_INT( false, parseDropAmountInputToCommand( &command, &inputBuffer, 8, &uiData ) );
    TEST_ASSERT_EQUAL_INT( -1, command.dropCounts[command.currentDropIdx] );

    inputBuffer.lastInput = inputBuffer.mappings.inputs[CONTEXT_AMOUNT][COMMAND_1];
    TEST_ASSERT_EQUAL_INT( true, parseDropAmountInputToCommand( &command, &inputBuffer, 8, &uiData ) );
...
void testParseInput( void )
{
    ...
    TEST_ASSERT_EQUAL_INT( false, updateCommandFromInput( &command, &inputBuffer, game.board.size, &uiData ) );
    inputBuffer.lastInput = inputBuffer.mappings.inputs[CONTEXT_INPUT_DEFAULT][COMMAND_FLAT];
    TEST_ASSERT_EQUAL_INT( true, updateCommandFromInput( &command, &inputBuffer, game.board.size, &uiData ) );
