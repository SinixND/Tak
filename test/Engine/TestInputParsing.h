...
void testParseInputActionType( void )
{
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_ACTION_TYPE][COMMAND_PLACE];
...
void testParseInputStoneType( void )
{
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_STONE_TYPE][COMMAND_FLAT];
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_STONE_TYPE][COMMAND_CAPSTONE];
    TEST_ASSERT_EQUAL_INT( true, parseInputStoneType( &command, &inputBuffer ) );
...
void testParseInputFileX( void )
{
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_B];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_B, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_C];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_C, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_D];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_D, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_E];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_E, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_F];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_F, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_G];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( FILE_G, command.fileX );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_H];
    TEST_ASSERT_EQUAL_INT( true, parseInputFileX( &command, &inputBuffer ) );
...
void testParseInputRankY( void )
{
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_3];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
    TEST_ASSERT_EQUAL_INT( RANK_3, command.rankY );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_4];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
    TEST_ASSERT_EQUAL_INT( RANK_4, command.rankY );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_5];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
    TEST_ASSERT_EQUAL_INT( RANK_5, command.rankY );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_6];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
    TEST_ASSERT_EQUAL_INT( RANK_6, command.rankY );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_7];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
    TEST_ASSERT_EQUAL_INT( RANK_7, command.rankY );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_POSITION][COMMAND_8];
    TEST_ASSERT_EQUAL_INT( true, parseInputRankY( &command, &inputBuffer, &game ) );
...
void testParseInputDirection( void )
{
    ...
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_DIRECTION][COMMAND_UP];
    TEST_ASSERT_EQUAL_INT( true, parseInputDirection( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( DIR_UP, command.direction );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_DIRECTION][COMMAND_DOWN];
    TEST_ASSERT_EQUAL_INT( true, parseInputDirection( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( DIR_DOWN, command.direction );

    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_DIRECTION][COMMAND_LEFT];
    TEST_ASSERT_EQUAL_INT( true, parseInputDirection( &command, &inputBuffer ) );
...
void testParseInputFirstDropAmount( void )
{
    ...
    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_2];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( 2, command.dropCounts[0] );

    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_3];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( 3, command.dropCounts[0] );

    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_4];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( 4, command.dropCounts[0] );

    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_5];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( 5, command.dropCounts[0] );

    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_6];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
    TEST_ASSERT_EQUAL_INT( 6, command.dropCounts[0] );

    command.dropCounts[0] = -1;
    command.drops = 0;
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_AMOUNT][COMMAND_7];
    TEST_ASSERT_EQUAL_INT( true, parseInputFirstDropAmount( &command, &inputBuffer ) );
...
void testParseInput( void )
{
    ...
    TEST_ASSERT_EQUAL_INT( false, parseInput( &command, &inputBuffer, &game ) );
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_INPUT_FIRST][COMMAND_FLAT];
    TEST_ASSERT_EQUAL_INT( true, parseInput( &command, &inputBuffer, &game ) );
    inputBuffer.lastInput = inputBuffer.keymap.inputs[CONTEXT_INPUT_FIRST][COMMAND_A];
    TEST_ASSERT_EQUAL_INT( true, parseInput( &command, &inputBuffer, &game ) );
