...
{
    //* TODO: Abort if topStone == CAP || !CAP onto WALL
    ...
    //* Can it be placed?
    assert( ( stacks->types[stackIdx] != CAP ) && "Cant place on top of capstone" );
    assert( !( ( type != CAP ) && ( stacks->types[stackIdx] == WALL ) ) && "Only capstone can be played on walls" );
